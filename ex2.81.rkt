...
(exp 3 9)
(define cpx1 (make-complex-from-mag-ang 2 1))
(define cpx2 (make-complex-from-mag-ang 1 2))
(exp cpx1 cpx2)
; (let ((t1->t2 (get-coercion type1 type2))
;       (t2->t1 (get-coercion type2 type1)))
;     (cond
;         ((not (null? t1->t2)) (apply-generic op (t1->t2 a1) a2))
;         ((not (null? t2->t1)) (apply-generic op a1 (t2->t1 a2)))
; 타입에 맞는 연산을 찾지 못했을때 실행하는 타입 변환 과정에서
; t1->t2, t2->t1 모두 true이기 때문에 무한 루프가 발생한다.

; b

; 놔두는 편이 프로그램의 복잡도 면에서 낫다.
