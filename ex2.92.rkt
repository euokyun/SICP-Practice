...
(define (install-rectangular-package)
    ...
    (define (magnitude z) 
        (make-scheme-number (sqrt-number (add (square (real-part z)) (square (imag-part z))))))
...
(define (install-polar-package)
    ...
    (define (make-from-real-imag x y) 
        (cons (sqrt-number (add (square x) (square y))) (arctangent y x)))
...
(define (install-rational-package)
    ...
    (put 'equ? '(rational rational) (lambda (rat1 rat2) 
...
(define (2.86-complex-package)
    ...
    (put 'arctangent '(complex complex) 
        (lambda (x y) (make-scheme-number (atan (real-part x) (real-part y)))))
    'done)
    ...
    (define (sub-poly p1 p2)
        (let ((negative-term (make-term 0 -1)))
            (if (same-variable? (variable p1) (variable p2)) 
                (make-poly 
                    (variable p1) 
                    (add-terms (term-list p1) (mul-terms (term-list p2) (list negative-term))))
    ...
    (define (sub-poly p1 p2)
        (let ((negative-term (make-term 0 -1)))
            (if (same-variable? (variable p1) (variable p2)) 
                (make-poly 
                    (variable p1) 
                    (add-terms (term-list p1) (mul-terms (term-list p2) negative-term)))
                (error "Polys not in same var -- SUB-POLY" (list p1 p2)))))
    ...
    (define (dense->sparse p)
        (define (make-dense-term-to-sparse-term term-list)
            (define (make-dense-term-to-sparse-term-1 order terms)
                (if (>= order 0) 
                    (cons 
                        (list order (car terms)) 
                        (make-dense-term-to-sparse-term-1 (- order 1) (cdr terms)))
                    '()))
            (make-dense-term-to-sparse-term-1 (- (length term-list) 1) term-list))
        (if (eq? (type-tag p) 'sparse-poly) p
            ((get 'make 'sparse-poly) 
                (variable (contents p)) 
                (make-dense-term-to-sparse-term (term-list (contents p))))))
