...
(define (attach-tag type-tag contents)
    (cond
        ; fix raise, drop procedure infinity loop
        ((equal? type-tag 'scheme-number) (cons type-tag contents))
        ; --
            ...
            (if (not (null? proc)) (drop (apply proc (map contents args)))
                (let* ([levelmap (map (lambda (x) (list-index x number-tower)) type-tags)]
                       [coercedargs ((lambda (elems) 
                            (cond
                                ((any (lambda (x) (eq? x #f)) levelmap) (error "error: not in type tree -- APPLY-COERCION" (list op elems)))
                                ((list-equal? levelmap) (map raise elems))
                                (else (let ((maxlevel (apply max levelmap)))
                                    (apply map (lambda (e l) 
                                        (cond
                                            ((= l maxlevel) e)
...
(define (install-rational-package)
    ...
    ; fix human error on rational equ? procedure
...
(define (install-complex-package)
    ...
    (define (add-complex-to-schemenum z x) (make-complex-from-real-imag (add (real-part z) x) (imag-part z)))
...
(define (install-polynomial-package)
    ...
    (define (gcd-poly p1 p2) 
        (if (eq? (type-tag p1) (type-tag p2)) (greatest-common-divisor p1 p2)
            (greatest-common-divisor (dense->sparse p1) (dense->sparse p2))))
...
(greatest-common-divisor p1 p2) ; (polynomial sparse-poly x (2 -1) (1 1))
(greatest-common-divisor 206 40) ; 2
(greatest-common-divisor (make-rational 412 2) 40) ; 2